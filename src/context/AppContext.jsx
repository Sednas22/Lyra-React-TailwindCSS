...
import beneficiarios from "../data/beneficiarios.json";
...
function findBeneficiario(name) {
  if (!name) return null;
  const n = normalizeName(name);
  return beneficiarios.find(b => normalizeName(b.nome) === n) ?? null;
}
...
export function AppProvider({ children }) {
  ...
  const [healthData, setHealthData]    = useState(() => load("lyra_health",    null));
  ...
  const setSyncApp = useCallback((appId) => {
    ...
    const b = findBeneficiario(load("lyra_user_name", ""));
    if (b) {
      save("lyra_health", b);
      setHealthData(b);
    } else {
      save("lyra_health", null);
      setHealthData(null);
    }
  }, []);
  ...
  const getGoalProgress = useCallback((goalId) => {
    if (!healthData) return { valor: 0, meta: 100, pct: 0, unidade: "" };
    const nivelMult = { leve: 0.7, moderado: 1.0, intenso: 1.3 };
    const m = nivelMult[goalPrefs[goalId]?.nivel || "moderado"];

    const map = {
      alimentacao: { valor: healthData.calorias_consumidas, meta: Math.round(healthData.calorias_meta * m),            unidade: "kcal"  },
      sono:        { valor: healthData.sono_horas,          meta: parseFloat((healthData.sono_meta * (m < 1 ? 0.85 : m > 1 ? 1.1 : 1)).toFixed(1)), unidade: "h" },
      passos:      { valor: healthData.passos,              meta: Math.round(healthData.passos_meta * m),              unidade: "passos"},
      exercicio:   { valor: healthData.calorias_gastas,     meta: Math.round(healthData.calorias_exercicio_meta * m),  unidade: "cal"   },
      hidratacao:  { valor: healthData.agua_ml,             meta: Math.round(healthData.agua_meta * m),                unidade: "ml"    },
    };

    const g = map[goalId];
    if (!g) return { valor: 0, meta: 100, pct: 0, unidade: "" };
    const pct = Math.min(100, Math.round((g.valor / g.meta) * 100));
    return { ...g, pct, cumprida: pct >= 100 };
  }, [healthData, goalPrefs]);

  
  const isBeneficiario = !!findBeneficiario(userName);
